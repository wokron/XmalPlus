...
class GetFeatureMatrix:
	def __init__(self):
		...
		self.APKlist = list()
		self.df = pd.DataFrame(columns=self.featureList)
		self.label = list()
		self.MatrixLock = threading.Lock()
		self.APKlistLock = threading.Lock()
		self.logger = Log(self)
		# self.embedding = EmbeddingProcess(r".\embedding")
	...
	def getFeaturefromAPK(self,dataPath,apkName):
		...
		apkFeature = np.zeros(self.featureList.__len__(), dtype=int)
		try:		
	...
	def getFeaturefromDocument(self,dataPath,flag):
		...
		for apk in ll:
			...
			if apk not in self.APKlist:
				...
				new_row = pd.Series(features, index=self.featureList)
	...
	def getFeatureMatric(self,benDatapath,malDatapath):
		...
		return self.df, self.label, self.featureList
		

if __name__=='__main__':
	adapter = GetFeatureMatrix()
	df, label ,featureList = adapter.getFeatureMatric("", r"E:\multibug_collect")
	df.to_csv(r"..\Data\BenData\featureVector2.csv")
